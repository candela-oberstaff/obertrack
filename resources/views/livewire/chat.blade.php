<div class="flex h-[calc(100vh-4rem)] bg-white overflow-hidden" wire:poll.2s>
    <!-- Contacts Sidebar -->
    <div class="w-full md:w-1/3 lg:w-1/4 flex flex-col border-r border-gray-100 {{ $selectedUserId ? 'hidden md:flex' : 'flex' }}">
        <!-- Header -->
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
            @forelse($contacts as $contact)
                <button 
                    ...
                    class="w-full p-4 hover:bg-gray-50 transition-all duration-200 text-left flex items-center gap-3 border-b border-gray-50 last:border-0 group {{ $selectedUserId == $contact->id ? 'bg-primary/5' : '' }}"
                >
                    <div class="relative">
    ...
    <!-- Main Chat Area -->
    <div class="flex-1 flex flex-col bg-[#F3F4F6] relative {{ $selectedUserId ? 'flex' : 'hidden md:flex' }}">
        @if($selectedUserId)
            @php
            ...
            <!-- Chat Header -->
            <div class="px-4 py-3 bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-10 flex items-center justify-between shadow-sm">
                <div class="flex items-center gap-3">
                    <button wire:click="$set('selectedUserId', null)" class="md:hidden p-2 -ml-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors">
            ...
            <!-- Messages Stream -->
            <div class="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar scroll-smooth" id="messages-container">
                <div wire:loading wire:target="selectContact" class="w-full flex justify-center py-12">
                ...
                @if($messages && count($messages) > 0)
                    ...
                    @foreach($messages as $message)
                        ...
                        @if($lastDate !== $dateStr)
                            <div class="flex justify-center">
                                <span class="bg-gray-200/80 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-gray-600 shadow-sm border border-white/50">
                                    {{ $dateStr }}
                                </span>
                            </div>
                                ...
                                <div class="px-5 py-3 shadow-sm text-[15px] leading-relaxed 
                                    ...
                                    @if(isset($message->attachment_path) && $message->attachment_path)
                                        <div class="mt-3 -mx-2 mb-1">
                                            @php
                                                $isImg = \Illuminate\Support\Str::endsWith($message->attachment_path, ['.jpg', '.jpeg', '.png', '.gif', '.webp']);
                                            @endphp
                                            @if($isImg)
                                                <img src="{{ Storage::url($message->attachment_path) }}" class="rounded-xl w-full max-h-60 object-cover hover:opacity-95 transition cursor-pointer">
                                            @else
                                                <a href="{{ Storage::url($message->attachment_path) }}" target="_blank" class="flex items-center gap-3 p-3 bg-black/5 rounded-xl hover:bg-black/10 transition">
                                                    <div class="p-2 bg-white rounded-lg text-primary">
                                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                                    </div>
                                                    <div class="flex-1 min-w-0">
                                                        <p class="font-medium text-sm truncate opacity-90">Archivo adjunto</p>
                                                        <p class="text-xs opacity-70">Clic para descargar</p>
                                                    </div>
                                                </a>
                                            @endif
                                        </div>
                    ...
                    @endforeach
                @else
                    <div class="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500">
            ...
            <div class="p-4 bg-white border-t border-gray-100">
                ...
                >
                    ...
                    <button 
                        type="submit" 
                        class="p-3 bg-primary hover:bg-primary-hover text-white rounded-xl shadow-lg hover:shadow-xl shadow-primary/30 transition-all duration-300 transform hover:scale-105 active:scale-95 mb-1 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <svg class="w-6 h-6 ml-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                    </button>
                </form>
            </div>
        @else
