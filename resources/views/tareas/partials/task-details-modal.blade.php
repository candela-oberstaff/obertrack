                    ...
                    <div class="flex-1 flex flex-col overflow-y-auto bg-white relative">
                        ...
                        <div class="sticky top-0 z-10 bg-white/95 backdrop-blur-sm px-8 py-6 flex justify-between items-start">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <template x-if="selectedTask && (currentUser.id == selectedTask.created_by || currentUser.id == (selectedTask.created_by && selectedTask.created_by.id) || currentUser.tipo_usuario == 'empleador' || currentUser.is_superadmin || currentUser.is_manager) && !isEditingTask">
                                ...
                                <div class="flex gap-3 group max-w-full">
                                    ...
                                    <div class="flex-1 min-w-0 overflow-hidden max-w-full">
                                        ...
                                        <div class="flex gap-2 mt-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity" x-show="currentUser.id == comment.user_id && editingCommentId != comment.id">
                                            <button @click="startEditingComment(comment)" class="p-1 text-gray-400 hover:text-[#22A9C8] hover:bg-blue-50 rounded-full transition-colors" title="Editar comentario">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                </svg>
                                            </button>
                                            <button @click="confirmDeleteComment(comment.id)" class="p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors" title="Eliminar comentario">
                                                 <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
                                            </button>
